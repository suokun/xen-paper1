    ...
    struct vcpu *v, int new_state, s_time_t new_entry_time)
{
    ...
    //add by Kun
    //v->runstate.state_AB = v->state_AB;
}
...
void vcpu_sleep_nosync(struct vcpu *v)
{
    ...
    t1 = (unsigned)(time >> 32);
    TRACE_6D(TRC_SCHED_KUN_11, v->domain->domain_id, v->vcpu_id, v->runstate.state, vcpu_runnable(v), t1, t2);
    ...
    t2 = (unsigned)(time & 0xffffffffLL);
    t1 = (unsigned)(time >> 32);
    TRACE_6D(TRC_SCHED_KUN_12, v->domain->domain_id, v->vcpu_id, v->runstate.state, vcpu_runnable(v), t1, t2);
...
*/	time = NOW();
	...
	t1 = (unsigned)(time >> 32);
    	...
    	TRACE_5D(TRC_SCHED_KUN_33, v->domain->domain_id, v->vcpu_id, vcpu_runnable(v), t1, t2); 
...
/***********************[xballoon begin]*************************************/
    ...
    d1 = (unsigned)(time >> 32);
    TRACE_3D(TRC_SCHED_KUN_23, v->domain->domain_id, d1, d2);
...
/***********************[xballoon begin]*************************************/
static void do_sleep(struct sched_sleep sleep, struct vcpu *v)
...
*/
	//
        ...
        d2 = (unsigned) (time & 0xffffffffLL);
        d1 = (unsigned) (time >> 32);
        TRACE_5D(TRC_SCHED_DO_SLEEP_2, v->domain->domain_id, v->vcpu_id, d1, d2, sleep.timeout);
        //

        set_timer(&v->balloon_timer, NOW() + NANOSECS(sleep.timeout + 5000));
/*
        d2 = (unsigned) (v->balloon_timer.expires & 0xffffffffLL);
        d1 = (unsigned) (v->balloon_timer.expires >> 32); 
        TRACE_2D(TRC_SCHED_KUN_17, d1, d2);
    }
    */
}
/***********************[end]***************************************/
...
long vcpu_yield(void)
{
    ...
    d2 = (unsigned)(time & 0xffffffffLL);
    d1 = (unsigned)(time >> 32);
    TRACE_4D(TRC_SCHED_KUN_24, v->domain->domain_id, v->vcpu_id, d1, d2);
    //end
...
/***********************[xballoon begin]*************************************/    
	...
	if(cap == 0 || cap >= 100) {
		...
		//delta = (uint64_t)(((now - v->start_time) * (100 - cap)) / cap );
		delta = 300000;
	}
	//uint64_t delta2 =  (uint64_t)(((now - v->start_time) * (100 - cap)) / cap );
	//uint64_t delta = 300000;

	//TRACE_6D(TRC_SCHED_KUN_16, v->domain->domain_id, v->vcpu_id, 
	//			delta, now, v->start_time, cap);
...
/*    time = NOW();
    d2 = (unsigned)(time & 0xffffffffLL);
    d1 = (unsigned)(time >> 32);
    TRACE_4D(TRC_SCHED_KUN_17, prev->domain->domain_id, next->domain->domain_id, d1, d2);
*/    //end
...
/*    vcpu_runstate_get(prev, &runstate1); 
    ...
    vcpu_runstate_get(next, &runstate2); 
    cpu_time2 = runstate2.time[RUNSTATE_running];
...
/*    prev->this_end_time = prev->runstate.time[0];
    ...
    next->prev_start_time = next->this_start_time;
    next->prev_end_time = next->this_end_time;
    next->this_start_time = next->runstate.time[0];
*/
    //if(next->vcpu_id == 1 && next->domain->domain_id != 32767) {
/*    if(next->domain->domain_id == 1) {
	vcpu_running_time = next->prev_end_time - next->prev_start_time;
	sum_time = vcpu_running_time + (prev->this_end_time - prev->this_start_time);
	cap = SCHED_OP(VCPU2OP(next), getcap, next);
	if(cap == 0){
		cap = 100;
	}

	if(sum_time * cap / 100 > vcpu_running_time) {
		rest_time += sum_time * cap / 100 - vcpu_running_time;
	}else{
		rest_time += 0;
	}
    ...
    TRACE_6D(TRC_SCHED_KUN_28, 
	     prev->domain->domain_id, prev->processor, prev->prev_start_time, 
	     prev->prev_end_time, prev->this_start_time, prev->this_end_time);

    TRACE_6D(TRC_SCHED_KUN_29, 
	     next->domain->domain_id, next->processor, next->prev_start_time, 
	     next->prev_end_time, next->this_start_time, next->this_end_time);

    if(next->domain->domain_id == 1) {
	TRACE_4D(TRC_SCHED_KUN_30, vcpu_running_time, sum_time, cap, rest_time);    
    }
  */  //end
...
/*
    if(atomic_read(&prev->do_sleep_flag)) {
	cap = SCHED_OP(VCPU2OP(prev), getcap, prev);
	sleep = (time - prev->start_time)*100/cap - (time - prev->start_time);
	stop_timer(&prev->balloon_timer);
	set_timer(&prev->balloon_timer, NOW() + NANOSECS(300000));
    }*/
}
