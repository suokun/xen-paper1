...
static void burn_credits(struct csched_vcpu *svc, s_time_t now)
{
    ...
    //add by Kun
    TRACE_5D(TRC_SCHED_KUN_26, svc->vcpu->domain->domain_id, svc->vcpu->vcpu_id, atomic_read(&svc->credit), credits, svc->pri);
}
...
csched_vcpu_sleep(const struct scheduler *ops, struct vcpu *vc)
{
    ...
    TRACE_5D(TRC_SCHED_VCPU_SLEEP, vc->domain->domain_id, vc->vcpu_id, curr_on_cpu(vc->processor) == vc, d1, d2);
    //end
   ...
   //end 
    ...
    list_for_each_safe( iter_sdom, next_sdom, &prv->active_sdom )
    {
	    ...
	    //end 
		    ...
		    //TRACE_6D(TRC_SCHED_CAP, svc->vcpu->domain->domain_id, svc->vcpu->vcpu_id, sdom->cap, svc->pri, d1, d2);
		    TRACE_6D(TRC_SCHED_CAP, svc->vcpu->domain->domain_id, credit_cap, sdom->cap, svc->pri, d1, d2);
		    //end
	    ...
	    //add by Kun
  	    TRACE_6D(TRC_SCHED_KUN_27, svc->vcpu->domain->domain_id, svc->vcpu->vcpu_id, 
			    	atomic_read(&svc->credit), credit_fair, svc->pri, 2);
        }
    }
